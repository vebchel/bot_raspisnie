...
def format_schedule(para: str, predmet: str, auditoriya: str, prepodavatel: str) -> str:
    ...
    # Словарь смайликов для разных предметов (можете расширить!)
    emoji_for_subject = {
        "Математика": "📐",
        "Физика": "🧪",
        "Информатика": "💻",
...
async def get_schedule(group: str) -> str:
    """
    Получает расписание для указанной группы с сайта колледжа на сегодня асинхронно.
    """
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(SCHEDULE_WEBSITE_URL) as response:
            ...
            if group_link:
                ...
                async with session.get(group_url) as schedule_response:
                    schedule_response.raise_for_status()
                    schedule_html = await schedule_response.text()
...
async def get_week_schedule(group: str) -> str:
    """
    Получает расписание для указанной группы с сайта колледжа на ближайшую неделю асинхронно.
    """
    ...
    try:
        ...
        logger.exception(f"Неизвестная ошибка при парсинге расписания для группы {group}: {e}")
        schedule_text = f"Произошла внутренняя ошибка при обработке расписания для **{group}**. 🛠️"
